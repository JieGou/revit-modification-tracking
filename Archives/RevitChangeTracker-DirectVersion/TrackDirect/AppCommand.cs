...
namespace TrackDirect
{
    class AppCommand : IExternalApplication
    {
        ...
        public static FooRequestHandler Handler { get; set; }
        ...
        private void BuildUI(UIControlledApplication uiApp)
        {
            ...
            var dataTrack = new PushButtonData("btnSnapshot", "Run\nTrack", assemblyPath, typeof(CmdTrackDirect).FullName);
        ...
        {
            if (CmdTrackDirect._startState != null)
                runTrack(source);
        }
        private static void OnDocumentOpened(object source, DocumentOpenedEventArgs args)
        {
            ...
            if (isAutoTrackEventDocumentOpen && canAutoRun  && CmdTrackDirect._startState == null)
                runTrack( source);
        ...
        private static void OnDocumentSaving(object sender, DocumentSavingEventArgs args)
        {
            Document doc = args.Document;
            CollectAutoTrackSetting(doc);
            if (isAutoTrackEventSave && canAutoRun && CmdTrackDirect._startState != null)
            {
                runTrack(sender);
                runTrack(sender);
            }
        ...
        private static void OnDocumentSynchronizing(object sender, DocumentSynchronizingWithCentralEventArgs args)
        {
            Document doc = args.Document;
            CollectAutoTrackSetting(doc);
            if (isAutoTrackEventSynchronize && canAutoRun  && CmdTrackDirect._startState != null)
            {
                runTrack(sender);
                runTrack(sender);
            }
        ...
        {
            CmdTrackDirect cmdTrackChange = new CmdTrackDirect();
