...
namespace Metamorphosis.Utilities
{
    public class SettingsFile
    {
        ...
        public static IList<SettingsFile> GetFiles()
        {
            ...
            foreach (string file in allFiles)
            {
                try
                {
                    ...
                    SettingsFile f = new SettingsFile(file);


                    foreach (XmlElement node in doc.SelectNodes("//Category"))
                    {
                        Settings s = new Settings() { Name = node.Attributes["name"].Value, CategoryId = Int32.Parse(node.Attributes["id"].Value), Enabled = (node.Attributes["enabled"].Value == "1") };
                        f.Settings.Add(s);
                    }
                    files.Add(f);
                }
        ...
        public static SettingsFile GetFileByName(string name)
        {
            var list = GetFiles();
