...
namespace TrackChanges
{
    ...
    {
        public static PushButton TrackChangesButton { get; set; }
        ...
        private void Application_ViewActivated(object sender, Autodesk.Revit.UI.Events.ViewActivatedEventArgs e)
        {
            ...
            TrackChangesUserControl.Instance.TextBlockProjectTitle.Text = app.ActiveUIDocument.Document.Title;
        }


        public Result OnShutdown(UIControlledApplication a)
        {
            TrackChangesButton.Enabled = true;
            return Result.Succeeded;
        }
        ...
        private void OnButtonCreate(UIControlledApplication application)
        {
            ...
            string dllLocation = Path.Combine(executableLocation, "TrackChanges.dll");

            // Create two push buttons

            PushButtonData buttondata = new PushButtonData("TrackChangesBtn", buttonText, dllLocation, "TrackChanges.Command");
            buttondata.ToolTip = buttonTooltip;

            BitmapImage pb1Image = new BitmapImage(new Uri("pack://application:,,,/TrackChanges;component/Resources/icon.ico"));
            buttondata.LargeImage = pb1Image;

            var ribbonPanel = RibbonPanel(application);


            TrackChangesButton = ribbonPanel.AddItem(buttondata) as PushButton;
